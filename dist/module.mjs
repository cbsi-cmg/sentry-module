...
function mapIntegrationToImportName(key) {
    ...
    ...getEnabledIntegrations(options.serverIntegrations).map((name) => {
      ...
      try {
        if (isServerCoreIntegration(name)) {
          ...
          return Object.keys(opt).length ? Sentry[importName](opt) : Sentry[importName]();
        } else {
...
async function initializeServerSentry(nuxt, moduleOptions, logger) {
  if (process.sentry) {
    return;
  }
  let release;
  ...
  if (canInitialize(moduleOptions)) {
    Sentry.init(config);
    nuxt.hook("render:setupMiddleware", (app) => app.use(() => Sentry.setupExpressErrorHandler(app)));
  }
}
